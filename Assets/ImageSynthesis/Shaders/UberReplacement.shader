...
// remap depth: [0 @ eye .. 1 @ far] => [0 @ eye .. xxx @ far]
inline float Linear01FromEyeToAbsolueFromEye(float depth01)
{
	float far = _ProjectionParams.z;
	return depth01 * far;
}
...
float4 Output(float depth01, float3 normal)
{
	...
	else if (_OutputMode == 2) // DepthCompressed
	{
		...
		float absoluteZFromNear = Linear01FromEyeToAbsolueFromEye(depth01);
		//float k = 0.25; // compression factor
		//return pow(linearZFromNear, k);
		//return absoluteZFromNear;
		return float4(absoluteZFromNear, absoluteZFromNear, absoluteZFromNear, 1.0f);
	}
	else if (_OutputMode == 3) // DepthMultichannel
	{
		...
		uint lowBits = uint(frac(depth01 * 256) * 256);
		uint midBits = uint(frac(depth01 - lowBits / 256) * 256);
		uint highBits = uint(frac((depth01 - midBits - lowBits / 256) / 256) * 256);
		return uint4(1, lowBits, midBits, highBits);
	}
